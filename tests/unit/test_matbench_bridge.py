...
# ---------------------------------------------------------------------------
# Status helpers — used by the GUI to surface matbench-genmetrics
# availability honestly. The GUI must never claim evaluation support when
# the dependency is missing.
# ---------------------------------------------------------------------------

class TestMatbenchStatus:

    def test_is_available_returns_bool(self):
        from nominal_drift.datasets.matbench_bridge import (
            is_matbench_genmetrics_available,
        )
        assert isinstance(is_matbench_genmetrics_available(), bool)
    ...
    def test_status_returns_dict_with_expected_keys(self):
        ...
        status = matbench_genmetrics_status()
        assert isinstance(status, dict)
        assert set(status.keys()) == {"available", "version", "import_error"}

    def test_status_available_matches_is_available(self):
        from nominal_drift.datasets.matbench_bridge import (
            is_matbench_genmetrics_available,
            matbench_genmetrics_status,
        )
        assert matbench_genmetrics_status()["available"] == is_matbench_genmetrics_available()

    def test_status_unavailable_has_import_error(self, monkeypatch):
        """When unavailable, import_error MUST be populated (no silent claim)."""
        import nominal_drift.datasets.matbench_bridge as mb
        monkeypatch.setattr(mb, "_MATBENCH_GENMETRICS_AVAILABLE", False)
        monkeypatch.setattr(
            mb, "_MATBENCH_IMPORT_ERROR",
            "ModuleNotFoundError: No module named 'matbench_genmetrics'",
        )
        status = mb.matbench_genmetrics_status()
        assert status["available"] is False
        assert status["import_error"]
        assert "matbench" in status["import_error"].lower()
