...
def render():
    ...
    # Actions
    if s.uses_figshare_loader:
        # MPTS-52 and other figshare datasets: download + normalise is one atomic step
        st.info(
            "ℹ️ **MPTS-52** uses `mp-time-split` to fetch ~40 k structures "
            "from **figshare** (~150 MB compressed). There is no separate CSV download step — "
            "clicking the button below downloads and indexes the dataset in one pass."
        )
        if st.button(
            "📦 Download + Normalise from figshare",
            disabled=s.is_normalised,
            key=f"norm_{dataset}",
        ):
            with st.spinner(
                f"Downloading MPTS-52 from figshare and normalising … "
                f"(first run may take 2–5 min)"
            ):
            ...
            if ok:
                ...
                st.error(msg)
                st.divider()
                st.markdown("**If this keeps failing**, install mp-time-split and run manually:")
                st.code(info.manual_instructions, language="text")
    else:
        # CSV-based datasets: separate Download and Normalise steps
        ...
        # Always show manual import path for CSV datasets — never hide it
    ...
    # -----------------------------------------------------------------------
    # Crystal evaluation lane — matbench-genmetrics availability (real check)
    # -----------------------------------------------------------------------
    st.divider()
    st.subheader("Crystal Evaluation Lane")
    st.caption(
        "Generative-model evaluation via matbench-genmetrics "
        "(Validity · Coverage · Novelty · Uniqueness on MPTS-52). "
        "Status below is a real import check, not a claim."
    )

    from nominal_drift.datasets.matbench_bridge import matbench_genmetrics_status
    mg_status = matbench_genmetrics_status()

    if mg_status["available"]:
        version = mg_status["version"] or "unknown"
        st.success(
            f"✅ `matbench-genmetrics` is installed and importable "
            f"(version: `{version}`). "
            f"Evaluation API: "
            f"`nominal_drift.datasets.matbench_bridge.evaluate_generated`."
        )
    else:
        err = mg_status["import_error"] or "module not found"
        st.error(
            "🚫 **`matbench-genmetrics` is NOT available in this environment.**\n\n"
            "Generative crystal evaluation (Validity / Coverage / Novelty / "
            "Uniqueness) is **not currently usable**. The bridge code in "
            "`nominal_drift/datasets/matbench_bridge.py` exists, but calling "
            "`evaluate_generated(...)` will raise `ImportError` until the "
            "dependency is installed.\n\n"
            f"**Import error:** `{err}`\n\n"
            "To enable: `pip install matbench-genmetrics` "
            "(requires a C/Fortran toolchain for numpy and a compatible "
            "Python version — currently pinned to numpy 1.26.x, which does "
            "not yet build on Python 3.14)."
        )
