...
DISCLAIMER: str = (
    "Continuum-driven illustrative animation — NOT atomistic.  "
    "No precipitation / carbide / phase kinetics modelled.  "
    "Particle colours encode the solved C(x,t) Crank-Nicolson field only."
)
...
class MicrostructureConfig:
    ...
    n_secondary : int
        Number of secondary-species particles shown near the interface to
        illustrate concentration of a coupled species.  Their motion is
        driven by the solved C(x,t) field — NOT by a precipitation or phase
        kinetics model.  No carbide, nitride, or second-phase formation is
        computed.
    boundary_colour : str
    ...
    depletion_zone_fraction : float
        Fraction of the domain (from x=0) highlighted as the depletion zone
        (schematic shading only — no precipitation model).  Set to 0.0 to
        disable shading.
    depletion_threshold : float
        Depletion-index threshold [0–1] above which secondary particles become
        visible as a schematic illustration.  Threshold crossing does NOT imply
        a thermodynamic phase boundary.
    show_legend : bool
    ...
    depletion_zone_fraction: float = 0.25
...
def animate_microstructure(
    ...
    *,
    ...
    # Depletion zone indicator (schematic shading near boundary — NOT a precipitation model)
    if config.depletion_zone_fraction > 0:
        pz_end = boundary_nm + config.depletion_zone_fraction * (x_max_nm - boundary_nm)
    ...
    for role, pop in pops:
        ...
        if role == "primary":
            ...
            # Secondary particles: schematic illustration of a coupled species
            # near the interface.  Visibility driven by solver C(x,t) only —
            # no precipitation / phase kinetics model.
    ...
    # 7. Animation update
    # ------------------------------------------------------------------
    pz_end_nm = boundary_nm + config.depletion_zone_fraction * (x_max_nm - boundary_nm)
    ...
    def _update(frame_num: int):
        ...
        if "secondary" in scatters:
            ...
            # This is a schematic illustration only — no precipitation / phase
            # kinetics are computed.  The threshold is NOT a thermodynamic
            # solubility limit.
            in_zone = pop_s.x < pz_end_nm
            above_thresh = d_s > config.depletion_threshold
...
def _infer_secondary_element(primary: str) -> Optional[str]:
    """Return a secondary species to visualise alongside the primary element.

    These pairings are chosen for schematic illustration only — they do NOT
    imply that carbide formation, nitride precipitation, gamma-prime
    nucleation, or any other second-phase reaction is modelled.  No
    thermodynamic / CALPHAD / kinetic-Monte-Carlo model is involved.
    """
    # Schematic illustration pairings (metallurgically motivated colour choice)
    pairings = {
        "Cr": "C",    # Cr-C pairing: illustrates sensitisation zone (schematic)
        "C":  "Cr",   # C-Cr pairing: illustrates carbon diffusion context
        "N":  "Cr",   # N-Cr pairing: illustrates nitride-zone context
        "Ni": "Al",   # Ni-Al pairing: illustrates gamma/gamma-prime context
        "Al": "Cu",   # Al-Cu pairing: illustrates age-hardening context
        "Cu": "Sn",   # Cu-Sn pairing: illustrates tin segregation context
        "O":  "Ti",   # O-Ti pairing: illustrates oxide-scale context
        "Ti": "O",    # Ti-O pairing: illustrates oxide formation context
    }
