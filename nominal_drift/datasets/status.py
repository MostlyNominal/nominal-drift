...
class DatasetStatus:
    ...
    @property
    def uses_figshare_loader(self) -> bool:
        """True for datasets that download via figshare/mp-time-split (no CSV files)."""
        return not bool(self.info.expected_raw_files)

    @property
    def is_raw_complete(self) -> bool:
        # Figshare/MPTimeSplit datasets (mpts-52) have NO expected CSV files.
        # For them "raw complete" is always True — the download happens
        # automatically inside the normalisation step, so the Normalise
        # button must never be gated behind a "raw complete" check.
        if self.uses_figshare_loader:
            return True
    ...
    def download_status_label(self) -> str:
        ...
        # Figshare/MPTimeSplit datasets: download + normalise is one step
        if self.uses_figshare_loader:
            return "📦 not yet fetched — click ⚙️ Normalise to download from figshare (~150 MB)"
