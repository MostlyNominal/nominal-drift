...
_MATBENCH_IMPORT_ERROR: str | None = None  # populated on first failed import


def is_matbench_genmetrics_available() -> bool:
    """Return True iff ``matbench_genmetrics`` can actually be imported.

    This is the single source of truth for the GUI / status layer.  It
    performs a real import (cached after the first call) so the answer
    reflects what the running interpreter can do, not what we hope is
    installed.
    """
    global _MATBENCH_GENMETRICS_AVAILABLE, _MATBENCH_IMPORT_ERROR
    if _MATBENCH_GENMETRICS_AVAILABLE is None:
        try:
            import matbench_genmetrics  # noqa: F401
            _MATBENCH_GENMETRICS_AVAILABLE = True
        except Exception as exc:  # ImportError, but also build/runtime errors
            _MATBENCH_GENMETRICS_AVAILABLE = False
            _MATBENCH_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"
    return bool(_MATBENCH_GENMETRICS_AVAILABLE)


def matbench_genmetrics_status() -> dict:
    """Return a structured status record for the GUI/status layer.
    ...
    import_error : str | None
        Human-readable error message if the import failed.
    """
    available = is_matbench_genmetrics_available()
    version: str | None = None
    if available:
        try:
            import matbench_genmetrics as _mg
            version = getattr(_mg, "__version__", None)
        except Exception:
            version = None
    return {
        "available": available,
        "version": version,
        "import_error": _MATBENCH_IMPORT_ERROR,
    }


def _check_matbench() -> None:
    if not is_matbench_genmetrics_available():
        raise ImportError(
            "matbench-genmetrics is required for evaluation.  "
            "Install it with: pip install matbench-genmetrics. "
            f"(Last import error: {_MATBENCH_IMPORT_ERROR})"
        )
