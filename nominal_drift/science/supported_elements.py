...
def get_supported_for_matrix(matrix: str) -> frozenset[str]:
    ...
    Strict, no-fake-fallback behaviour: if *matrix* has no entries in the
    Arrhenius database the function returns an **empty frozenset**.  It
    intentionally does NOT fall back to the full ``DIFFUSION_SUPPORTED``
    set, because Arrhenius constants are matrix-specific (Cr-in-austenite
    is not a valid proxy for Cr-in-Al-FCC or Cr-in-Ni-base).  Returning
    the global set would let unrelated material systems silently inherit
    steel constants — exactly the kind of fake support this module is
    here to prevent.
    ...
    -------
    frozenset[str]
        Elements with validated D₀/Qd for *matrix*.  Empty if none.
    """
    return _PER_MATRIX.get(matrix, frozenset())


def known_matrices() -> frozenset[str]:
    """Return the set of matrices that have *any* validated Arrhenius constants.

    Useful for honest reporting and tests: distinguishes "matrix exists in
    DB but element X is missing" from "matrix is entirely unsupported".
    """
    return frozenset(_PER_MATRIX.keys())
